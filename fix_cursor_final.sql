-- Final fix for cursor pagination duplicates
-- The key issue: we need to get the LAST row of the result set in sort order
-- And the comparison must match our DESC sort order

DO $$
BEGIN
  -- Drop and recreate the function with proper cursor logic
...
BEGIN
  ...
  -- Paginate with proper keyset comparison
  paged AS (
    SELECT *
    FROM base
    WHERE (
  ...
  -- Get last row for cursor (must maintain same sort order)
  last_row AS (
    SELECT sort_key, id::text as id_text
    FROM paged
    ORDER BY sort_key ASC, id ASC  -- OPPOSITE of main sort to get LAST row
    LIMIT 1
  )

  -- Build cursor only if we got a full page
  SELECT
    CASE
