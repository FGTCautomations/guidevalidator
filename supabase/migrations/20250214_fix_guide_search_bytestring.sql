-- Fix ByteString error in guide search with special characters
-- The issue is in cursor decoding when search queries contain special characters
...
BEGIN
  ...
  -- Build text search query - use simple text matching instead of tsvector for special chars
  IF p_q IS NOT NULL AND LENGTH(TRIM(p_q)) > 0 THEN
    BEGIN
      v_q := plainto_tsquery('simple', unaccent(p_q));
    EXCEPTION WHEN OTHERS THEN
      -- If unaccent fails, just use plainto_tsquery without unaccent
      v_q := plainto_tsquery('simple', p_q);
    END;
  ELSE
    v_q := NULL;
  END IF;
  ...
  WITH base AS (
    ...
    WHERE country_code = UPPER(p_country)
      ...
      -- Text search with fallback to ILIKE if tsvector search fails
      AND (
        v_q IS NULL
        OR search_text @@ v_q
        OR (p_q IS NOT NULL AND (
          name ILIKE '%' || p_q || '%'
          OR headline ILIKE '%' || p_q || '%'
        ))
      )
