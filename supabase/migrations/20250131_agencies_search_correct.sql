-- ============================================================
-- AGENCIES, DMCS, AND TRANSPORT SEARCH SYSTEM
-- Uses existing agencies table with type column
-- ============================================================
...
DROP FUNCTION IF EXISTS api_agencies_search CASCADE;
DROP FUNCTION IF EXISTS api_dmcs_search CASCADE;
DROP FUNCTION IF EXISTS api_transport_search CASCADE;

-- ============================================================
-- MATERIALIZED VIEW: agencies_browse_v
-- ============================================================
...
-- ============================================================
-- MATERIALIZED VIEW: dmcs_browse_v
-- ============================================================
...
-- ============================================================
-- MATERIALIZED VIEW: transport_browse_v
-- ============================================================
...
-- ============================================================
-- RPC FUNCTION: api_agencies_search
-- ============================================================
...
BEGIN
  ...
  WITH filtered AS (
    SELECT *
    FROM dmcs_browse_v v
    WHERE (p_country IS NULL OR v.country_code = p_country)
      AND (p_languages IS NULL OR v.languages && p_languages)
      AND (p_specializations IS NULL OR v.specialties && p_specializations)
      AND (p_q IS NULL OR v.search_text @@ plainto_tsquery('simple', unaccent(p_q)))
      AND (p_min_rating IS NULL OR v.rating >= p_min_rating)
      AND (NOT p_license_only OR v.verified = true)
      AND (p_cursor IS NULL OR (v.sort_key, v.name) < (v_cursor_sort_key, v_cursor_name))
...
-- ============================================================
-- RPC FUNCTION: api_transport_search
-- ============================================================
...
BEGIN
  ...
  WITH filtered AS (
    SELECT *
    FROM transport_browse_v v
    WHERE (p_country IS NULL OR v.country_code = p_country)
      AND (p_languages IS NULL OR v.languages && p_languages)
      AND (p_service_types IS NULL OR v.service_types && p_service_types)
      AND (p_q IS NULL OR v.search_text @@ plainto_tsquery('simple', unaccent(p_q)))
      AND (p_min_rating IS NULL OR v.rating >= p_min_rating)
      AND (NOT p_license_only OR v.verified = true)
      AND (p_cursor IS NULL OR (v.sort_key, v.name) < (v_cursor_sort_key, v_cursor_name))
  ...
  SELECT jsonb_build_object(
    ...
    'serviceTypes', (
      ...
      FROM (
        SELECT UNNEST(service_types) as service_type, COUNT(*) as cnt
        FROM base
...
DO $$
BEGIN
  RAISE NOTICE '✅ Agencies, DMCs, and Transport search system created successfully!';
  RAISE NOTICE '';
  RAISE NOTICE 'Created:';
  RAISE NOTICE '  - Materialized view: agencies_browse_v';
  RAISE NOTICE '  - Materialized view: dmcs_browse_v';
  RAISE NOTICE '  - Materialized view: transport_browse_v';
  RAISE NOTICE '  - RPC function: api_agencies_search()';
  RAISE NOTICE '  - RPC function: api_dmcs_search()';
  RAISE NOTICE '  - RPC function: api_transport_search()';
  RAISE NOTICE '';
  RAISE NOTICE 'Next steps:';
  RAISE NOTICE '  1. Verify views have data:';
  RAISE NOTICE '     SELECT COUNT(*) FROM agencies_browse_v;';
  RAISE NOTICE '     SELECT COUNT(*) FROM dmcs_browse_v;';
  RAISE NOTICE '     SELECT COUNT(*) FROM transport_browse_v;';
  RAISE NOTICE '  2. Test RPC functions';
