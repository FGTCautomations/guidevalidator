-- Fix pagination for guides search - cursor comparison must match sort direction
-- For DESC sorts (featured), use < instead of >
...
BEGIN
  ...
  WITH base AS (
    SELECT
      ...
      CASE p_sort
        ...
        ELSE (
          -- Featured ordering: is_featured (10000) + is_activated (5000) + quality score
  ...
  -- Paginate with cursor (keyset pagination - direction depends on sort)
  paged AS (
    SELECT *
    FROM base
    WHERE (
      ...
      OR (
        -- For DESC sorts (featured, relevance), use < comparison
        -- For ASC sorts (price, rating), use > comparison
        CASE
          WHEN p_sort IN ('featured', 'relevance') THEN
            (sort_key, id::text) < (
...
COMMENT ON FUNCTION api_guides_search IS 'Faceted guide search with Fixed pagination (cursor direction matches sort order)';
