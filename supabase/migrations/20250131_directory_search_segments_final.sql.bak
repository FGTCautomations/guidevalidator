...
-- Uses application_data JSONB for fields not in agencies table
...
SELECT
  ...
  a.languages,
  a.specialties,
  p.application_data->>'logoUrl' as logo_url,
  p.application_data->>'websiteUrl' as website_url,
...
FROM agencies a
INNER JOIN profiles p ON a.id = p.id
LEFT JOIN LATERAL (
  SELECT
...
-- ============================================================================
-- DMCS BROWSE VIEW & SEARCH (same as agencies, just filtered by role)
-- ============================================================================
...
-- DMC search RPC function (identical to agencies)
...
-- ============================================================================
-- Note: transport_providers table may not exist, so just use profiles + application_data
...
SELECT
  ...
  p.full_name as name,
  CASE
    WHEN jsonb_typeof(p.application_data->'languages') = 'array' THEN
      ARRAY(SELECT jsonb_array_elements_text(p.application_data->'languages'))
    ELSE '{}'::text[]
  END as languages,
  '{}'::text[] as service_types,  -- Will be extracted from application_data in future
  p.application_data->>'logoUrl' as logo_url,
  p.application_data->>'websiteUrl' as website_url,
  p.application_data->>'registrationCountry' as country_code,
  COALESCE(pr.avg_overall_rating, 0) as rating,
  COALESCE(pr.review_count, 0) as review_count,
  25::bigint as sort_key,  -- Default sort key
  to_tsvector('simple', unaccent(COALESCE(p.full_name, ''))) as search_text
FROM profiles p
LEFT JOIN LATERAL (
  SELECT
