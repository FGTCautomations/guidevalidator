...
-- FIXED: agencies.id IS the profile_id (agencies table uses profile id as primary key)
...
-- Check if transport_providers table exists, if not use profiles with role='transport'
CREATE MATERIALIZED VIEW IF NOT EXISTS transport_browse_v AS
SELECT
  p.id,
  p.full_name as name,
  COALESCE(tp.languages, '{}') as languages,
  COALESCE(tp.service_types, '{}') as service_types,
  tp.logo_url,
  tp.website_url,
  tp.registration_country as country_code,
  COALESCE(pr.avg_overall_rating, 0) as rating,
  COALESCE(pr.review_count, 0) as review_count,
  (
    CASE
      WHEN tp.license_verified THEN 50
      ELSE 0
    END
  )::bigint as sort_key,
  to_tsvector('simple', unaccent(COALESCE(p.full_name, ''))) as search_text
FROM profiles p
LEFT JOIN transport_providers tp ON tp.id = p.id
LEFT JOIN LATERAL (
  SELECT
    AVG(r.overall_rating) as avg_overall_rating,
    COUNT(*) as review_count
