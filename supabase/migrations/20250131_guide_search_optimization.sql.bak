-- ============================================
-- Guide Search Optimization Migration
...
SELECT
  ...
  -- Experience (you may need to add this column if not exists)
  -- COALESCE(g.years_experience, 0) as experience_years,
...
-- ============================================
-- 4. LOCATION INDEXES (for region/city filtering)
-- ============================================

-- Index on guide_regions for location filtering
CREATE INDEX IF NOT EXISTS idx_guide_regions_guide_region
  ON guide_regions(guide_id, region_id);

-- Index on guide_cities for location filtering
...
BEGIN
  ...
  -- Get location-filtered guide IDs if region or city specified
  IF p_region_id IS NOT NULL THEN
    SELECT ARRAY_AGG(guide_id) INTO v_location_filter
    FROM guide_regions WHERE region_id = p_region_id;
  ELSIF p_city_id IS NOT NULL THEN
    SELECT ARRAY_AGG(guide_id) INTO v_location_filter
    FROM guide_cities WHERE city_id = p_city_id;
  END IF;
  ...
  WITH base AS (
    SELECT
      ...
      CASE p_sort
        ...
        WHEN 'experience' THEN 0::bigint -- Add experience_years if column exists
        ELSE (
...
ANALYZE guide_regions;
ANALYZE guide_cities;
ANALYZE guides;
ANALYZE profiles;
