...
export async function POST(request: NextRequest) {
  try {
    ...
    const { token, licenseNumber, email, password } = body;

    // Validation
    if (!token || !licenseNumber || !email || !password) {
    ...
    if (claimToken.license_number.trim().toLowerCase() !== licenseNumber.trim().toLowerCase()) {
      return NextResponse.json(
        { error: "License number does not match" },
        { status: 400 }
      );
    }
