...
import { searchAgencies, type AgencySearchParams } from "@/lib/agencies/api";
import { AgencyFilters } from "@/components/agencies/agency-filters";
import { AgencyResults } from "@/components/agencies/agency-results";
  ...
  return {
    ...
    specialties:
      typeof searchParams.specialties === "string"
        ? searchParams.specialties.split(",").filter(Boolean)
        : undefined,
    nicheFocus:
      typeof searchParams.nicheFocus === "string"
        ? searchParams.nicheFocus.split(",").filter(Boolean)
        : undefined,
...
export default async function AgenciesPage({
  ...
  const availableCountries = await fetchAvailableCountries("agencies");
        ...
        <div className="mx-auto flex w-full max-w-6xl flex-col gap-6">
          <div className="space-y-2">
            ...
            <p className="text-sm text-foreground/70 sm:text-base">
              Find trusted travel agencies around the world
            </p>
          </div>
          ...
          <CountryFilter
            ...
            placeholder="Choose a country to explore agencies"
          />
          ...
          <div className="rounded-xl border border-foreground/10 bg-white p-12 text-center shadow-sm">
            <div className="mx-auto max-w-md space-y-3">
              ...
              <p className="text-sm text-foreground/70">
                Choose a country above to view available agencies
              </p>
            </div>
          </div>
        </div>
  ...
  try {
    const data = await searchAgencies(filterParams);
    ...
    return (
      <div className="flex flex-col gap-10 bg-background px-6 py-12 text-foreground sm:px-12 lg:px-24">
        <div className="mx-auto flex w-full max-w-6xl flex-col gap-6">
          ...
          <AgencyFilters facets={data.facets} currentFilters={filterParams} />
