...
import { fetchDmcProfile } from "@/lib/profile/dmc";
...
function formatCoverage(description?: string | null, coverageSummary?: string | null) {
  if (description) return description;
  if (coverageSummary) return coverageSummary;
  return null;
}

type DmcProfilePageProps = {
  params: { locale: string; id: string };
};

export default async function DmcProfilePage({ params }: DmcProfilePageProps) {
  ...
  const coverage = formatCoverage(profile.description, profile.coverageSummary);
      ...
      <div className="mx-auto w-full max-w-5xl space-y-6">
        ...
        <ProfileHeader
          ...
          breadcrumbs={[
            ...
            { label: "DMCs", href: `/${locale}/directory?tab=dmcs` },
          ]}
          badges={badges}
          actions={[{ label: t("actions.contact"), href: `/${locale}/contact?dmc=${profile.id}` }]}
        />
        ...
        {profileRating && profileRating.totalReviews > 0 && (
          <div className="flex items-center gap-4 rounded-xl border border-foreground/10 bg-white/60 p-6">
            <div className="flex flex-col gap-2">
              ...
              {/* Category Ratings Breakdown for DMCs (rated by guides) */}
        ...
        <div className="space-y-6 border-t border-foreground/10 pt-8">
          <div className="flex items-center justify-between">
            ...
            {canReview && (
              <Link
                href={`/${locale}/reviews/submit?revieweeId=${id}&revieweeType=dmc` as Route}
