-- Final fix for cursor pagination duplicates - Version 2
-- Fixed: PostgreSQL CTE scoping issue
...
BEGIN
  ...
  -- Build complete response with all CTEs in one WITH clause
  WITH base AS (
    SELECT
  ...
  -- Build final response in a single SELECT
  SELECT json_build_object(
    ...
    'nextCursor',
      CASE
        WHEN (SELECT page_count FROM page_stats) = p_limit THEN
          encode(
            convert_to(
              (SELECT last_sort_key FROM page_stats)::text || ':' || (SELECT last_id FROM page_stats),
              'UTF8'
            ),
            'base64'
          )
