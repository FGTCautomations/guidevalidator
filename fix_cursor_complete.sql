-- Complete working fix for cursor pagination duplicates
-- Fixed: All CTEs must be used within the same WITH clause
...
BEGIN
  ...
  -- Build complete response with all CTEs together
  WITH base AS (
    SELECT
  ...
  -- Get the last row for cursor generation - MUST be in same WITH clause
  last_row AS (
    SELECT
      sort_key,
      id::text as id_text,
    ...
    FROM paged
    ORDER BY sort_key ASC, id ASC  -- Reverse order to get LAST row
    LIMIT 1
  )
