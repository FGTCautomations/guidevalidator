...
import type { AgencySearchParams, FacetCount } from "@/lib/agencies/api";
...
export function AgencyFilters({
  ...
  }, [currentFilters.languages, currentFilters.specialties]);
  ...
  const toggleLocalFilter = (filterType: 'languages' | 'specialties', value: string) => {
  ...
  const applyPendingFilters = () => {
    ...
    if (pendingSpecialties.size > 0) {
      params.set("specialties", Array.from(pendingSpecialties).join(","));
    } else {
  ...
  return (
    <div className="space-y-6 rounded-lg border bg-white p-6 shadow-sm">
      ...
      {facets.specialties && facets.specialties.length > 0 && (
        <div>
          <label className="mb-2 block text-sm font-medium text-foreground">
            Services Offered ({facets.specialties.length})
          </label>
          <div className="flex max-h-48 flex-col gap-2 overflow-y-auto">
