-- Simple fix for cursor pagination duplicates
-- The issue: cursor should capture the LAST row's values in sort order
-- Not just MAX values which don't respect sort order
...
BEGIN
  ...
  -- Paginate (FIXED: proper keyset pagination)
  paged AS (
    SELECT *
    FROM base
    WHERE (
  ...
  -- Get last row values for cursor (FIXED)
  SELECT
    sort_key,
    id::text
  INTO v_last_sort_key, v_last_id
  FROM paged
  ORDER BY sort_key DESC, id DESC
  LIMIT 1;

  -- Build cursor only if we got a full page
  IF (SELECT COUNT(*) FROM paged) = p_limit AND v_last_id IS NOT NULL THEN
